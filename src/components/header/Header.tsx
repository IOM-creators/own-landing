...
const Header = () => {
  ...
  return (
    <header className={`sticky top-0 z-50 bg-gray-dark w-full  transition-transform transform ${scrollingDown ? 'translate-y-0' : '-translate-y-full'}`}>
      <Icon icon="logo" />
      <LanguageSelector />
...
const LanguageSelector = () => {
  ...
  return (
    <div>
      <select defaultValue={defaultLanguage} onChange={(e) => changeLanguage(e)} name="languageSelect" id="languageSelect">
        {languages.map((language: ILanguage) => {
          return <option key={language.shortName} value={language.shortName}>{language.name}</option>
        })}
      </select>
    </div>
  )
...
const HeaderNavigation: React.FC<IHeaderNavigation> = ({ navigation }) => {
  ...
  return <nav>
    <ul>
      {navigation && navigation.map((navItem: INavigation) => {
        return <li key={navItem.textId}>
          <a href="/" data-scroll-to={navItem.scrollTo} onClick={(e) => scrollTo(e)}>
            {t(`${navItem.textId}`)}
          </a>
        </li>
      })}
