...
export async function fetchPageContent(slug: string, fetchOnlyHeaderFooter = false, items?: any[]) {
    ...
    const pageContentItems = items || (await (async () => {
    ...
    const sectionsWithComponents = await Promise.all(
        pageContentItems.map(async (item: any) => {
            const { data: sectionData } = await client.query({
